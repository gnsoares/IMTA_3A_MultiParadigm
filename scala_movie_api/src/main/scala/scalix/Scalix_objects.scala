...
// functional objects
case class Actor(name: String, surname: String)
case class Movie(id: Int, title: String)
case class Director(id: Int, name: String)

object ScalixObjects
    extends App,
      Config(
  ...
  // cache maps
  var actorIdMap: Map[Actor, Int] = Map()
  var actorMoviesMap: Map[Int, Set[Movie]] = Map()
  var movieDirectorMap: Map[Int, Director] = Map()
  ...
  def findActorId(name: String, surname: String): Option[Int] =
    actorIdMap.get(Actor(name, surname)) match
      case None =>
  ...
  private def findActorIdFromFile(name: String, surname: String): Option[Int] =
    ...
    parse(source) \ "id" match
      case JArray(JInt(id) :: _) =>
        actorIdMap += (Actor(name, surname) -> id.toInt)
        Some(id.toInt)
      case _ => None
  ...
  private def findActorMoviesFromAPIOrFile(actorId: Int): Set[Movie] =
  ...
  private def findMovieDirectorFromAPIOrFromFile(
    ...
    (parse(source) \ "crew")
      ...
      case Some(Person(id: Int, name: String, _)) =>
        movieDirectorMap += (movieId -> Director(id, name))
        Some(id, name)
      case _ => None
  ...
  def collaboration(actor1: FullName, actor2: FullName): Set[(String, String)] =
    ...
    actor1Movies
      ...
      .map(movie =>
        (
          findMovieDirector(movie.id) match {
